resource "azurerm_storage_account" "storage_account" {
  ...
  # share_properties (soft-delete) only supported on StorageV2, not FileStorage
  ...
  # blob_properties only supported on StorageV2/BlobStorage, not FileStorage
...
# blob_diagnostics only applicable for StorageV2 — FileStorage has no blobServices endpoint
...
resource "azurerm_storage_management_policy" "log_retention" {
  count              = var.log_retention_enabled && var.account_kind != "FileStorage" ? 1 : 0
...
# Provisioned v2: Set IOPS and Throughput via AzAPI
# azurerm_storage_share does not support provisionedIops / provisionedBandwidthMibps.
# These are only available via Azure REST API 2024-01-01.
# This resource is a NO-OP when billing_model = "payg".
resource "azapi_update_resource" "file_share_provisioned_v2" {
  for_each = {
    for k, v in var.file_shares : k => v
    if v.billing_model == "provisioned_v2" && (v.provisioned_iops != null || v.provisioned_bandwidth_mibps != null)
  }
