output "security_groups" {
  ...
  value = merge(
    { for k, v in aws_security_group.sg : k => { id = v.id, name = v.name, arn = v.arn, type = "new" } },
    { for k, v in data.aws_security_group.existing : k => { id = v.id, name = v.name, arn = v.arn, type = "existing" } }
  )
}
...
output "new_security_groups" {
  ...
  value = { for k, v in aws_security_group.sg : k => { id = v.id, name = v.name, arn = v.arn } }
}

output "existing_security_groups" {
  description = "Map of existing security groups that had rules added"
  value = { for k, v in data.aws_security_group.existing : k => { id = v.id, name = v.name, arn = v.arn } }
}
